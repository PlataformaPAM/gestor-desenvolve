...
export async function POST(req: Request) {
  ...
  try {
    const report = await buildOperacionalSnapshot({
      ...
      access: relatorioAccessFromGate(gate, RELATORIOS_OPERACIONAL_RESOURCE),
    });
    const subject = report.assunto || `Relatório operacional - ${report.resumo.reportTitulo}`;
    ...
    const send = await sendDocumentoEmail({
      to: toEmail,
      subject,
      text: `Segue relatório operacional em anexo.\nTipo: ${report.resumo.reportTitulo}\nPeríodo: ${report.resumo.periodoInicio} até ${report.resumo.periodoFim}`,
      html: `<p>Segue relatório operacional em anexo.</p><p><strong>Tipo:</strong> ${report.resumo.reportTitulo}</p><p><strong>Período:</strong> ${report.resumo.periodoInicio} até ${report.resumo.periodoFim}</p>`,
      attachmentFilename: `relatorio-operacional-${Date.now()}.pdf`,
