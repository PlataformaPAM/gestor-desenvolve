...
import { alertasAccessGate, alertaUsuarioScope, allowedAlertModules } from "@/lib/server/alertas-access";
...
export async function GET(req: Request) {
  ...
  const gate = await alertasAccessGate(req, "ver");
  ...
  if (allowed.size === 0) {
    ...
    return ok({ alertas: [], data: { alertas: [] } });

  }



  const whereUsuario = alertaUsuarioScope(gate.session);
